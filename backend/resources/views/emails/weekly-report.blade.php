  ...
  <tr>
    <td style="background:#1F3A5F;border-radius:12px 12px 0 0;padding:28px 32px;">
      ...
      <div style="margin-top:20px;">
        <div style="color:#fff;font-size:22px;font-weight:700;">{{ $org->nom }}</div>
        <div style="color:#93c5fd;font-size:13px;margin-top:4px;">
          Semaine du {{ $debut->locale('fr')->isoFormat('D MMMM') }} au {{ $fin->locale('fr')->isoFormat('D MMMM YYYY') }}
        </div>
    ...
    <td style="background:#fff;padding:0 32px 8px;">
      ...
      <div style="margin-top:28px;margin-bottom:8px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="8">
          <tr>
            <td width="48%" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px;text-align:center;">
              <div style="font-size:24px;font-weight:800;color:#1F3A5F;">{{ $fmt($data['ca_semaine']) }}</div>
              <div style="font-size:11px;color:#64748b;margin-top:4px;text-transform:uppercase;letter-spacing:.5px;">CA semaine</div>
              <div style="margin-top:8px;font-size:13px;font-weight:700;color:{{ $varCouleur }};">{{ $varTexte }}</div>
              ...
              <table width="100%" cellpadding="0" cellspacing="6">
                <tr>
                  <td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;">
                    ...
                    <div style="font-size:18px;font-weight:700;color:#1F3A5F;">{{ $data['nb_ventes'] }}</div>
                  </td>
                </tr>
                <tr>
                  <td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;">
                    <div style="font-size:12px;color:#64748b;">Ticket moyen</div>
                    <div style="font-size:18px;font-weight:700;color:#1F3A5F;">{{ $fmt($data['ticket_moyen']) }}</div>
                  </td>
                </tr>
              </table>
            </td>
        ...
        <div style="margin-top:8px;padding:10px 14px;background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;">
          <span style="font-size:12px;color:#92400e;">🏆 Meilleur jour : <strong>{{ $data['meilleurJour'] }}</strong></span>
        </div>
        @endif
      </div>

      @endif {{-- fin if nb_ventes --}}

      {{-- ─── Section 2 : Top 5 plats ─── --}}
      @if($data['topPlats']->isNotEmpty())
      <div style="margin-top:24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
          ...
          @foreach($data['topPlats'] as $i => $plat)
          <tr style="background:{{ $loop->even ? '#f8fafc' : '#fff' }};">
            <td style="padding:9px 12px;font-size:13px;color:#94a3b8;border-bottom:1px solid #f1f5f9;">{{ $i + 1 }}</td>
            <td style="padding:9px 12px;font-size:13px;font-weight:600;color:#1e293b;border-bottom:1px solid #f1f5f9;">{{ $plat->nom }}</td>
            <td style="padding:9px 12px;font-size:13px;color:#1e293b;text-align:right;border-bottom:1px solid #f1f5f9;">{{ number_format($plat->nb_vendus, 0, ',', ' ') }}</td>
            <td style="padding:9px 12px;font-size:13px;font-weight:600;color:#1F3A5F;text-align:right;border-bottom:1px solid #f1f5f9;">{{ $fmt($plat->ca_genere) }}</td>
          </tr>
          @endforeach
        </table>
      </div>
      ...
      @if($data['stockAlerte']->isNotEmpty())
      <div style="margin-top:24px;">
        <div style="font-size:11px;font-weight:700;color:#C8860A;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">
          📦 Stock à commander
        </div>
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
          ...
          @foreach($data['stockAlerte'] as $prod)
          @php
            $pct = $prod->seuil_alerte > 0 ? ($prod->quantite / $prod->seuil_alerte) * 100 : 0;
            $couleur = $pct <= 25 ? '#dc2626' : '#d97706';
          @endphp
          <tr style="background:{{ $loop->even ? '#fff7f7' : '#fff' }};">
            <td style="padding:9px 12px;font-size:13px;font-weight:600;color:#1e293b;border-bottom:1px solid #fef2f2;">{{ $prod->nom }}</td>
            <td style="padding:9px 12px;font-size:13px;text-align:right;border-bottom:1px solid #fef2f2;">
              <span style="color:{{ $couleur }};font-weight:700;">{{ number_format($prod->quantite, 3, ',', ' ') }}</span>
              <span style="color:#94a3b8;font-size:11px;"> {{ $prod->unite_mesure }}</span>
            </td>
            <td style="padding:9px 12px;font-size:13px;color:#64748b;text-align:right;border-bottom:1px solid #fef2f2;">{{ number_format($prod->seuil_alerte, 3, ',', ' ') }} {{ $prod->unite_mesure }}</td>
          </tr>
          @endforeach
        </table>
      </div>
        ...
        <table width="100%" cellpadding="0" cellspacing="8">
          <tr>
            <td width="32%" style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:22px;font-weight:800;color:#16a34a;">{{ $data['nbNouveauxClients'] }}</div>
            ...
            <td width="32%" style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:22px;font-weight:800;color:#d97706;">{{ $data['nbClientsArdoise'] }}</div>
            ...
            <td width="32%" style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:16px;font-weight:800;color:#dc2626;">{{ $fmt($data['totalArdoises']) }}</div>
      ...
      <div style="margin-top:24px;">
        ...
        @endphp
        <div style="margin-bottom:8px;padding:12px 16px;background:{{ $bg[0] }};border-left:4px solid {{ $bg[1] }};border-radius:0 8px 8px 0;font-size:13px;color:#1e293b;line-height:1.5;">
          {{ $conseil['texte'] }}
        </div>
        @endforeach
      </div>
    ...
    <td style="background:#1F3A5F;border-radius:0 0 12px 12px;padding:20px 32px;">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="right" style="vertical-align:middle;">
            <div style="color:#475569;font-size:11px;">Envoyé le {{ now()->locale('fr')->isoFormat('D MMM YYYY') }}</div>
          </td>
        </tr>
      </table>
    </td>
